...
function SdmReport() {
  ...
  const [uploadedSignatureUrl, setUploadedSignatureUrl] = useState(null); // State for uploaded signature preview
  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      ...
      setUploadedSignatureUrl(previewUrl);
      
      try {
        const formData = new FormData();
        formData.append('applicationId', id);
        formData.append('LekhpalSignature', file);

        const response = await axios.put(
          'https://lampserver.uppolice.co.in/validation-report/uploadsignatureSDM',
          formData,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
        ...
        if (response.data.status) {
          ...
          // Refresh the report data to get updated signature
          ...
          if (refreshResponse.data.status && refreshResponse.data.data) {
            setApplication(refreshResponse.data.data);
            // Keep the preview URL to show the uploaded signature
            // setUploadedSignatureUrl(null); // Commented out to keep showing the signature
          }
        } else {
          ...
          <div className='table-responsive mt-table'>
            <table className="table table-bordered custom-table" aria-label="Application Validation Report">
              <tbody>
                ...
                <tr>
                  <th scope="col">SDM हस्ताक्षर</th>
                  <td scope="col" colSpan={2}>
                    <div className='signature-img'>
                      {application.signature_url ? (
                        <img
                          src={fileUrl(application.signature_url)}
                          alt="SDM Signature"
                ...
                <tr>
                  <th scope="col">TEHSILDAR हस्ताक्षर</th>
                  <td scope="col" colSpan={2}>
                    <div className='signature-img'>
                      {application.TehsildarSignature ? (
                        <img
                          src={fileUrl(application.TehsildarSignature)}
                          alt="TEHSILDAR Signature"
                          className='signature-img'
                        />
                      ) : (
                        <img
                ...
                <tr>
                  <th scope="col">NAYAK TEHSILDAR हस्ताक्षर</th>
                  <td scope="col" colSpan={2}>
                    <div className='signature-img'>
                      {application.NayakSignature ? (
                        <img
                          src={fileUrl(application.NayakSignature)}
                          alt="Nayak Signature"
                          className='signature-img'
                        />
                      ) : (
                ...
                <tr>
                  <th scope="col">REVANUE INCHARGE हस्ताक्षर</th>
                  <td scope="col" colSpan={2}>
                    <div className='signature-img'>
                      {application.RiSignature ? (
                        <img
                          src={fileUrl(application.RiSignature)}
                          alt="Nayak Signature"
                          className='signature-img'
                        />
                      ) : (
                ...
                <tr>
                  <th scope="col">LEKHPAL हस्ताक्षर</th>
                  <td scope="col" colSpan={2}>
                    <div className='mb-3 no-print'>
                      <input
                        ...
                        disabled={uploading}
                        placeholder={uploading ? "Uploading..." : "Select signature file"}
                      />
                      {uploadSuccess && (
                    ...
                    <div className='signature-img'>
                      {/* Priority: 1. Server signature, 2. Preview signature, 3. Default signature */}
                      {application.LekhpalSignature ? (
                        <img
                          src={fileUrl(application.LekhpalSignature)}
                          alt="Lekhpal Signature"
                          className="signature-img"
                        />
