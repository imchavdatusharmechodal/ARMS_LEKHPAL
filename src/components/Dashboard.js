...
const totalPages = Math.ceil(filteredApplications.filter(app => app.forwarded_to_sdm).length / itemsPerPage);
  ...
  const fetchApplications = async (token) => {
    try {
      const response = await axios.get('https://lampserver.uppolice.co.in/arms/lekhpal-applications', {
        headers: {
      ...
      // Handle the response structure based on the API response
      let apps = [];
      if (response.data && response.data.status && Array.isArray(response.data.data)) {
        apps = response.data.data.filter(Boolean);
      } else if (Array.isArray(response.data)) {
        apps = response.data.filter(Boolean);
      }
                      ...
                      {paginatedApplications
                        ...
                        .map((app, index) => (
                          <tr key={app.id}>
                            ...
                            <td>
                              <Link to={`/filled-pdf/${app.id}`} className='btn btn-primary btn-sm'>
                                View Application
                              </Link>
                            </td>
                            <td>
                          {submittedReports.includes(String(app.id)) ? (
                            <Link to={`/sdm-report/${app.id}`}  className="btn btn-success btn-sm">
                              View Report
                            </Link>
                          ) : (
